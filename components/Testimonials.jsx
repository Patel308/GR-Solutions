...
export default function Testimonials() {
  return (
    <section className="section section-testimonials py-24">
      <div className="container">
        <div className="section-title section-centered-title mb-12 text-center">
          <span className="badge inline-flex rounded-full bg-primary/10 px-4 py-2 text-sm font-black text-primary">CUSTOMER TRUST</span>
        ...
        <div className="grid-3 grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {testimonials.map((item) => (
            <article className="card rounded-card border border-black/5 bg-white p-10 shadow-oldMd transition duration-300 hover:-translate-y-1 hover:shadow-oldLg" key={item.name}>
              <Image src={item.image} alt={`${item.name} testimonial`} width={90} height={90} className="testimonial-avatar mb-6 rounded-full" />
